...
<style>
    ...
    .select:focus {
        ...
        box-shadow: 0 0 0 4px rgba(0, 192, 127, .15);
    }
    ...
    .spinner {
        ...
        height: 18px;
        border-radius: 50%;
        border: 3px solid rgba(0, 192, 127, .2);
...
<div class="ab-page">
    ...
    <div class="search-card">
        <form id="paymentSearchForm" method="get" action="index.php" autocomplete="off">
            <input type="hidden" name="action" value="owner_payment_search" />

            <div class="search-grid">
                <div class="field">
                    <label>Mã booking / tên khách / số điện thoại</label>
                    <input class="input" type="text" name="keyword" placeholder="Tìm theo tên khách hàng, sdt" value="<?= htmlspecialchars($_GET['keyword'] ?? '') ?>" />
                </div>
                ...
                <div class="field">
                    ...
                    <select class="select" name="payment_status">
                        ...
                        <option value="Pending" <?= $ps === 'Pending' ? 'selected' : '' ?>>Chưa thanh toán</option>
                        <option value="Confirmed" <?= $ps === 'Confirmed' ? 'selected' : '' ?>>Đã thanh toán</option>
    ...
    <div id="paymentResults">
        <?php
        // Non-AJAX initial render uses GET endpoint.
        // If controller uses AJAX only, vẫn có thể gọi bằng browser.
...
<script>
    (function() {
        ...
        if (!form || !results || !resetBtn || !loading) {
            // Khi render partial kết hợp layout khác nhau có thể thiếu 1 số element.
            // Không throw lỗi runtime.
            console.warn('[OwnerPaymentSearch] Missing DOM elements, skip JS init');
            return;
        }
        ...
        async function fetchResults(page = 1) {
            ...
            params['page'] = page;
            params['action'] = 'owner_payment_search_ajax';
        ...
        form.addEventListener('submit', function(e) {
            ...
            // giữ lại giá trị filter trong URL
            const u = new URL(window.location.href);
            Object.keys(params).forEach(k => {
                u.searchParams.set(k, params[k]);
            });
            u.searchParams.set('action', 'owner_payment_search');
        ...
        // delegate pagination links from partial
