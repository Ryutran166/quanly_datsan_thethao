...
<style>
:root{--primary:#00c07f;--primary-dark:#00a06a;--primary-soft:#e6faf3;--dark:#0f172a;--mid:#475569;--muted:#94a3b8;--border:#e2e8f0;--surface:#fff;--page-bg:#f1f5f9;}
...
.table-head{padding:14px 18px;border-bottom:1.5px solid var(--border);}
...
<div class="rp-page">
    ...
    <div class="table-card">
        <div class="table-head"><h3>Doanh thu theo sân</h3></div>
...
(function(){
    ...
    new Chart(document.getElementById('chartMonth'), {
        ...
        data: {
            ...
            datasets: [{ label: 'Doanh thu (đ)', data: <?= json_encode(array_column($revenueByMonth, 'revenue')) ?>, backgroundColor: 'rgba(0,192,127,.25)', borderColor: primary, borderWidth: 2, borderRadius: 6 }]
        },
        options: { plugins: { legend: { display: false } }, scales: { y: { ticks: { callback: v => (v/1000000).toFixed(1)+'M' }, grid: { color: '#f1f5f9' } }, x: { grid: { display: false } } } }
    });
    ...
    if (methodData.length > 0) {
        new Chart(document.getElementById('chartMethod'), {
            type: 'doughnut',
            data: { labels: <?= json_encode($methodLabels) ?>, datasets: [{ data: methodData, backgroundColor: ['#3b82f6','#00c07f'], borderWidth: 0, hoverOffset: 6 }] },
            options: { plugins: { legend: { position: 'bottom', labels: { font: { weight: '700' }, padding: 14 } } }, cutout: '68%' }
        });
    }
    ...
    new Chart(document.getElementById('chartDay'), {
        ...
        data: {
            ...
            datasets: [{ label: 'Doanh thu (đ)', data: <?= json_encode(array_column($revenueByDay, 'revenue')) ?>, borderColor: primary, backgroundColor: 'rgba(0,192,127,.08)', fill: true, tension: 0.4, pointRadius: 4, pointBackgroundColor: primary }]
        },
        options: { plugins: { legend: { display: false } }, scales: { y: { ticks: { callback: v => (v/1000000).toFixed(1)+'M' }, grid: { color: '#f1f5f9' } }, x: { grid: { display: false } } } }
    });
    <?php endif; ?>
